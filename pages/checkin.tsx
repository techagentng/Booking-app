...
interface CheckinGuest {
  id: string;
  reservationId: number; // Numeric ID for API calls
  bookingId: string; // Display reference (confirmation number)
...
export default function CheckIn() {
  ...
  const apiReservations = reservationsData?.data?.data || reservationsData?.data || [];
  ...
  const checkIns: CheckinGuest[] = apiReservations.map((res: any) => {
    ...
    return {
    id: `CI-${res.id}`,
    reservationId: res.id, // Store numeric ID for API calls
    bookingId: res.confirmation_number || res.booking_id || `BK-${res.id}`,
  ...
  const handleCheckout = async () => {
    if (!selectedGuest?.reservationId) {
      console.error('Invalid reservation ID:', selectedGuest);
      toast.error('Invalid reservation. Please try again.');
      return;
    }

    try {
      await checkoutMutation.mutateAsync({
        id: selectedGuest.reservationId,
